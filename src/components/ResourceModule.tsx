    ...
    <div className="grid grid-cols-12 gap-8 h-full pr-4 pb-12">
      ...
      <div className="col-span-3 bg-white rounded-[40px] border border-gray-100 shadow-sm flex flex-col p-10 overflow-hidden">
        <div className="mb-10">
           ...
           <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Geo-Segment Matrix</p>
        </div>
        ...
        <button className="mt-10 py-5 bg-gray-900 text-white rounded-3xl text-[10px] font-black uppercase tracking-[0.3em] hover:bg-black transition-all">
           View Global Pool
        </button>
      </div>

      {/* Number List Area - 9 Columns */}
      <div className="col-span-9 bg-white rounded-[40px] border border-gray-100 shadow-sm flex flex-col overflow-hidden">
         <div className="p-10 border-b border-gray-50 flex items-center justify-between bg-gray-50/20">
            <div>
               <h4 className="text-3xl font-black text-gray-900 uppercase tracking-tight">{selectedProvince} 分区池</h4>
               <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Resource Logical Mapping Pipeline</p>
            </div>
            <div className="flex gap-4">
               <button className="px-8 py-4 bg-gray-100 text-gray-900 rounded-2xl text-[10px] font-black uppercase tracking-widest">Filter</button>
               <button className="px-8 py-4 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-blue-500/20">Allocate New</button>
            </div>
         </div>
            ...
            <table className="w-full text-left border-collapse">
               <thead>
                  <tr className="bg-gray-50/50 border-b border-gray-100">
                     <th className="px-10 py-6 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">终端号码 / ISP</th>
                     <th className="px-10 py-6 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">单向流转状态 [Mock]</th>
                     <th className="px-10 py-6 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">业务挂载主体</th>
                     <th className="px-10 py-6 text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] text-right">时间戳</th>
                  </tr>
               </thead>
                       ...
                       <td className="px-10 py-8">
                          {num.status === 'buffering' && (
                            <div className="inline-block px-4 py-2 bg-amber-500 text-white rounded-xl shadow-lg shadow-amber-500/20">
                               <span className="text-[9px] font-black italic uppercase tracking-widest">缓冲期 - 剩余 {num.countdown}</span>
                            </div>
                          )}
                          {num.status === 'frozen' && (
                            <div className="inline-block px-4 py-2 bg-gray-400 text-white rounded-xl">
                               <span className="text-[9px] font-black italic uppercase tracking-widest">冻结池 - 冷却中 ({num.countdown})</span>
                            </div>
                          )}
                          {num.status === 'public' && (
                            <div className="inline-block px-4 py-2 bg-emerald-500 text-white rounded-xl shadow-lg shadow-emerald-500/20">
                               <span className="text-[9px] font-black italic uppercase tracking-widest">公共池 - 可分配</span>
                            </div>
                          )}
                          {num.status === 'normal' && (
                            <div className="inline-block px-4 py-2 bg-blue-600 text-white rounded-xl">
                               <span className="text-[9px] font-black italic uppercase tracking-widest">正常服役中</span>
                            </div>
                          )}
                       </td>
                       <td className="px-10 py-8">
                          <p className="text-xs font-black text-gray-900 uppercase">@{num.user}</p>
                          <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mt-1">Ref: {num.id}</p>
                       </td>
